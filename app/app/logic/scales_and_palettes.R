box::use(
  dplyr,
)

box::use(
  app / logic / constants
)

palette_list <- readRDS(file.path(constants$data_dir, "palette_list.rds"))

scale_fxs <- readRDS(file.path(constants$data_dir, "scale_fxs.rds"))

#' @export
seifa_scale_to_text <- scale_fxs$seifa_scale_to_text

#' @export
seifa_text_to_value <- scale_fxs$seifa_text_to_value

#' @export
seifa_text_choices <- c(seifa_scale_to_text(1:5), "NA")

#' @export
ra_scale_to_text <- scale_fxs$ra_scale_to_text

#' @export
ra_text_to_value <- scale_fxs$ra_text_to_value

#' @export
ra_text_choices <- ra_scale_to_text(0:4)

#' @export
pal_mins <- palette_list$palNum

#' @export
pal_hours <- palette_list$palNum_hours

#' @export
pal_index <- palette_list$paliTRAQI

#' @export
iTRAQI_levels <- palette_list$bins_index
